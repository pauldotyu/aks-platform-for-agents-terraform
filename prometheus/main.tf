resource "azurerm_monitor_data_collection_endpoint" "msprom" {
  ...
  name                = "MSProm-${var.random_pet_name}"
  kind                = "Linux"
}
...
resource "azurerm_monitor_data_collection_rule" "msprom" {
  resource_group_name         = var.resource_group_name
  location                    = var.resource_group_location
  name                        = "MSProm-${var.resource_group_location}-${var.aks_cluster_name}"
  ...
  data_flow {
    streams      = ["Microsoft-PrometheusMetrics"]
    destinations = [var.azure_monitor_workspace_name]
  }
}
...
resource "azurerm_monitor_data_collection_rule" "msci" {
  resource_group_name = var.resource_group_name
  location            = var.resource_group_location
  name                = "MSCI-${var.resource_group_location}-${var.aks_cluster_name}"
  kind                = "Linux"
